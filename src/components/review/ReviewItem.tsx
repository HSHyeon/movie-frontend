...
function ReviewItem({review}: { review: ReviewType }) {
    ...
    const onDelete = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axiosInstance.get(`/review/delete/${review.id}`).then((resp) => {
                    const {data} = resp
    ...
    return (
        <Card className="review-item shadow-sm p-3 my-3 text-start">
            <Card.Body>
                <Row className="align-items-center mb-2">
                  ...
                  {userId == review.writerId && (
                        <Col className="d-flex text-end align-items-center gap-2">
                            ...
                            <a className="text-danger" onClick={onDelete}>삭제</a>
                        </Col>
                    )}
                </Row>
